	...
	function bs_kenburn(arg)
	{
	  	...
	  	function bs_init()
		{
			...
			handle.find(".bs_slide").each(function(i)
			{
				...
				// saving original width/height for responsive
				$(this).find(".bs_layer").each(function(i){
					$(this).attr("o-width",$(this).width()).attr("o-height",$(this).height()).attr("o-lft",$(this).css("left")).attr("o-top",$(this).css("top")).attr("o-rgt",$(this).css("right")).attr("o-btm",$(this).css("bottom"));
				});
			});
			...
			if(option.skin=='transformer') pt=50;
	 		...
	 		if(!option.fullwidth)
	 			...
	 			handle.css("width",width).css("height",height).attr("o-width",windows.width).attr("o-height",windows.height).attr("o-font",pt);
			rate=width/height;
			...
			handle.find(".bs_operate").each(function(i)
			{
				...
				if(!cache1.find(".bs_image").hasClass("active"))
				{
					...
					cache.find(".bs_image").on('load',function()
					{
						...
						if(code==1)
							...
							$(this).attr("width",option.width*2).attr("height",option.height*2).css("margin-left",-parseInt(option.width)/2).css("margin-top",-parseInt(option.height)/2);
						$(this).addClass("active");
				...
				else
				{
					...
					if(code==1)
						...
						cache.find(".bs_image").attr("width",option.width*2).attr("height",option.height*2).css("margin-left",-parseInt(cache.find(".bs_image").width()-option.width)/2).css("margin-top",-parseInt(cache.find(".bs_image").height()-option.height)/2);
		...
		function bs_skin_setup()
		{
			switch(option.skin)
			{
			  ...
			  case 'jumbo':
				  option.caption_animation=2;
				  option.progressbartype='linear';
				  handle.find("#bs_shadow").html('<img src="img/skin/jumbo_shadow.png">');
			  ...
			  case 'transformer':
				  ...
				  option.navtype=1;
				  handle.append('<a id="left_btn" class="bs_nav">Previous Slide</a><a id="right_btn" class="bs_nav right_btn">Next Slide</a>');
				  handle.find("#bs_shadow").html('<img src="img/skin/transformer_shadow.png">');
			  break;
			  case 'beach':
				  option.caption_animation=3;
				  option.progressbartype='linear';
				  option.navtype=1;				
				  handle.append('<a id="left_btn" class="bs_nav">Previous Slide</a><a id="right_btn" class="bs_nav right_btn">Next Slide</a>');
				  handle.find("#bs_shadow").html('<img src="img/skin/beach_shadow.png">');
				  c_twidth=90;
			  break;
			}
		...
		function bs_bullet_setup(handle)
		{
			...
			{
				var twidth;
				if(option.skin=='jumbo') twidth=37*length;
				else twidth=27*2+33*length;
				...
				if(i==length-1)
				{
					...
					// calculate exact width of carousel
					$("#bs_bullet").css("width",twidth);
			...
			twidth=handle.find(".bs_bt_slide").width();
			if(option.skin=='jumbo') twidth=109;
			else if(option.skin=='sapphire') twidth=50;
			handle.find(".bs_bt_container").css("width",twidth*length);	 
		}
		...
		function bs_thumb_preview(arg){	  	
			...
			var twidth=handle.find(".bs_bt_slide").width();
			if(option.skin=="jumbo")
				handle.find('.bs_bt_preview').css("left",tleft-40);
			else if(option.skin=="starworld")
				handle.find('.bs_bt_preview').css("left",tleft+105);
			else
			...
			switch(t_flag)
			{
				  ...
				  case 0:
					  ...
					  handle.find('.bs_bt_container').css('margin-left',-twidth*(arg-1));
...
/*				handle.find(".bs_thumb").find(".bs_thumb_img").greyScale({
		...
		function bs_swipe_end()
		{
			...
			if(option.skin=="starworld")
				handle.find("#bs_numboard").html((target_index+1)+" / "+length);
			if(option.carousel!=false)
			{
				c_flag=0;
		...
		function bs_respond()
		{
			...
			handle.find(".bs_operate").each(function(i)
			{
				$(this).find(".bs_layer").each(function(){
					var width=$(this).attr("o-width");
					var height=$(this).attr("o-height");
					var lft=$(this).attr("o-lft");
					var rgt=$(this).attr("o-rgt");
					var top=$(this).attr("o-top");
					var btm=$(this).attr("o-btm");
					lft=(lft!="auto")?parseInt(lft)*ratio:lft;
					rgt=(rgt!="auto")?parseInt(rgt)*ratio:rgt;
					top=(top!="auto")?parseInt(top)*ratio:top;
					btm=(btm!="auto")?parseInt(btm)*ratio:btm;
					$(this).css("width",width*ratio).css("height",height*ratio).css("left",lft*ratio).css("right",rgt*ratio).css("top",top*ratio).css("bottom",btm*ratio);
				});
			});
		}
		//bs carousel refresh
		function bs_carousel_refresh()
		{
			var carousel_width=(option.width-100>c_twidth*length)?c_twidth*length:option.width-100;
			var left=(option.width-carousel_width)/2-8;
			//repositioning carousel and thumbnail
			handle.find("#bs_carousel").css("marginLeft",0).css("left",left).css("width",carousel_width);
			if(option.width-100>c_twidth*length) handle.find("#bs_tviewport").css("margin-left",0);
			thumb_rate=(handle.find("#bs_tviewport").width()-carousel_width)/(carousel_width-100);
		}
		//animation function
		function bs_animation(arg)
		{
			...
			if(!cache.find(".bs_image").hasClass("active"))
			{
				...
				if(code==1)
					...
					handle.find("#bs_next").find(".bs_image").attr("width",option.width*2).attr("height",option.height*2).css("margin-left",-parseInt(handle.find("#bs_next").find(".bs_image").width()-option.width)/2).css("margin-top",-parseInt(handle.find("#bs_next").find(".bs_image").height()-option.height)/2);
				bs_animate_init();			
			}
		}
		...
		function bs_layer_end_animate()
		{
			...
			handle.find("#bs_current").find(".bs_layer").removeClass("animation_done").each(function(i){
				...
				{
					$(this).clearQueue().stop().delay(delay).addClass(endfilter).css({"animation-duration":animtime+"ms","-webkit-animation-duration":animtime+"ms"}).css({"animation-delay":delay+"ms","-webkit-animation-delay":delay+"ms"}).animate({
						"marginLeft":"+="+ox+"px",
						"marginTop":"+="+oy+"px",
						"opacity":opacity,
						"zoom": zoom,
		...
		function bs_layer_animate()
		{
				...
				handle.find("#bs_current").find(".bs_layer").removeClass("animation_done").each(function(i){
					...
					if(starttype!="")
					{
						$(this).stop().clearQueue().delay(delay).addClass(startfilter).css({"animation-duration":animtime+"ms","-webkit-animation-duration":animtime+"ms"}).css({"animation-delay":delay+"ms","-webkit-animation-delay":delay+"ms"}).animate({
							"marginLeft":"+="+100+"px",
							"marginTop":"+="+oy+"px",
							"opacity":opacity,
							"scale": zoom,
						...
						{
							duration:parseInt(animtime),
							easing:easing,
							complete: function()
							{	
								startfilter=$(this).attr("start-filter-type");
								//animation is done
								if(startfilter=="")		//check if Css3 transition applied or not
								{
									starttype=$(this).attr("end-type");
									//if not
									if(starttype=="1"){
										var endtype=$(this).attr("end-type");
										var delay=$(this).attr("end-delay");
										...
										$(this).stop().clearQueue().delay(delay).addClass(endfilter).css({"animation-duration":animtime+"ms","-webkit-animation-duration":animtime+"ms"}).css({"animation-delay":delay+"ms","-webkit-animation-delay":delay+"ms"}).animate({
											"marginLeft":"+="+ox+"px",
											"marginTop":"+="+oy+"px",
											"opacity":opacity,
											"scale": zoom,
										...
										},
										{
											...
											{	
												...
												if(endfilter=="")
												{
													$(this).stop().clearQueue().addClass("animation_done").addClass("all_done");	
												}
												if(handle.find("#bs_current").find(".animation_done").length==length)
												{
													handle.find("#bs_current").find(".all_done").remove();
													//animation is done
													lock=0;
													option.onanimend();	
													if(option.responsive==true)	bs_respond();
								...
								//check if all layers animation are done							
								if(handle.find("#bs_current").find(".animation_done").length==length)
								{
									handle.find("#bs_current").find(".all_done").remove();
									//animation is done
						...
						// immediate disappear flag
						var i_dflag=0;
						// CSS3 transition ended
						$(this).bind("animationend webkitAnimationEnd oAnimationEnd MSAnimationEnd", function(){ 
							// remove first filter class
							if(i_dflag==1)
							{
								$(this).removeClass(endfilter); 
								$(this).stop().clearQueue().addClass("animation_done").addClass("all_done");
								if(handle.find("#bs_current").find(".animation_done").length==length)
								{
									handle.find("#bs_current").find(".all_done").remove();
							...
							$(this).removeClass(startfilter); 
							i_dflag=1;

							//animation is done check if continuous animation necessary
							if(starttype=="1"){	// if yes
			...
			/*handle.find("#bs_current").find(".bs_layer").removeClass("animation_done").each(function(i){
			    ...
			    $(this).delay(delay).queue(function(){
			    	if(i==0) $(this).css("opacity",1).addClass("flipInX");	
			    	else $(this).css("opacity",1).addClass("flipInY");	
			    });
			});*/
		...
		function bs_kenburn()
		{
			...
			var posx=(parseInt(option.width)/800);
			var posy=(parseInt(option.height)/800);
			var stepx=kposx/400;
			var stepy=kposy/400;
			var blur_index=0;
			...
			if(bs_isIE()!=false&&bs_isIE()<9)
			{
				...
				$("<img class='bs_canvas' width='"+(option.width*2)+"' src='"+cache.attr("src")+"'/>").insertAfter(cache);
			...
			else
			{
					...
					if(kzoom==1)
						...
						//ctx.drawImage(cache[0],-parseInt(cache.width()-option.width)/2,-parseInt(cache.height()-option.height)/2,cache.width()*2,cache.height()*2);
						ctx.drawImage(cache[0],-parseInt(option.width)/2,-parseInt(option.height)/2,option.width*2,option.height*2);
					function clear() {
						// Clear the canvas
						ctx.save();
						ctx.globalAlpha = 1;
					...
					function update()
					{
						...
						else if(kzoom==1)
						{
							...
							if(-parseInt(option.width)/2+posx*index+stepx*index>-10||-parseInt(option.height)/2+posy*index+stepy*index>-10||option.width/2-posx*index+stepx*index<0||option.height/2-posy*index+stepy*index<0)
							{
								clearInterval(kentimer);
								return;	
							}
						...
						if(kzoom==1){
							...
							ctx.drawImage(cache[0],-parseInt(option.width)/2+posx*index+stepx*index,-parseInt(option.height)/2+posy*index+stepy*index ,option.width*2-2*posx*index,option.height*2-2*posy*index);
							//ctx.filter='blur('+(20-blur_index)+'px)';
						}
					}
					kentimer=setInterval(update,50);
			}
		}
		...
		$(document).ready(function(){
			...
			handle.find("#bs_shadow").mousemove(function(){
				if(option.skin=='ios') handle.find("#bs_carousel").hide();
			});
			//mousemove
			...
			$(window).resize(function()
			{
				...
				if(option.responsive==true)
				{
					...
					//clearInterval(kentimer);
					//handle.find(".bs_canvas").remove();
					var temp=$(window).width()/(windows.width);
					bs_respond();
					//handle.find(".bs_operate").find(".bs_image").css("marginLeft",0).css("marginTop",0);
				}
				if(a_flag==1)
				{
					bs_stop();
